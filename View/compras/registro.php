...
<h2>Editar Compra</h2>
<form action="index.php?classe=Compra&metodo=update&id=<?= htmlspecialchars($compra['id']) ?>" method="POST" class="mt-4">
  ...
  <div class="mb-3">
    ...
    <input type="number" class="form-control" id="cliente_id" name="cliente_id" required
      value="<?= htmlspecialchars($compra['cliente_id']) ?>" />
  </div>
  ...
  <div class="mb-3">
    ...
    <input type="date" class="form-control" id="data" name="data" required
      value="<?= htmlspecialchars($compra['data']) ?>" />
  </div>

  <div class="mb-3">
    <label for="horario" class="form-label">Horário</label>
    <input type="time" class="form-control" id="horario" name="horario" required
      value="<?= htmlspecialchars($compra['horario']) ?>" />
  </div>

  <div class="mb-3">
    <label for="qtd" class="form-label">Quantidade</label>
    <input type="number" class="form-control" id="qtd" name="qtd" required min="1"
      value="<?= htmlspecialchars($compra['qtd']) ?>" />
  </div>
